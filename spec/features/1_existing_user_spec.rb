...
feature 'Existing users' do
  scenario 'can log in to see the cheeses index' do
    user = User.new(email: 'foo@bar.com', password: 'supersecure', password_confirmation: 'supersecure')
    user.save!
  ...
  scenario 'are taken back to the sign in form and shown a flash error if no such user exists' do
    visit root_path
    click_link 'Sign in'
